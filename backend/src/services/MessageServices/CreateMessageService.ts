...
const CreateMessageService = async ({
  ...
  if (existingMessage) {
    const existingWithAssociations = await Message.findOne({
      ...
      include: [
        "contact",
        {
          ...
          as: "ticket",
          include: [
            { model: Contact, attributes: ["id", "name", "number", "email", "profilePicUrl", "acceptAudioMessage", "active", "urlPicture", "companyId"], include: ["extraInfo", "tags"] },
    ...
    return existingWithAssociations || existingMessage;
  }
  ...
  const upsertData: any = { ...messageData, companyId };
  if (Object.keys(extraData).length > 0) {
    upsertData.dataJson = JSON.stringify(extraData);
  }

  await Message.upsert(upsertData);
