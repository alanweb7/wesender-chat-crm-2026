...
import GetUserGoogleCalendarIntegrationService from "../UserGoogleCalendarServices/GetUserGoogleCalendarIntegrationService";
...
const SyncEventService = async ({ schedule }: SyncEventRequest): Promise<void> => {
  ...
  if (!userId) {
    console.log(
      "SyncEventService - schedule sem userId definido, não é possível sincronizar",
      "scheduleId:",
      (schedule as any).id
    );
    return;
  ...
  const integration = await GetUserGoogleCalendarIntegrationService(userId);

  if (!integration) {
    console.log(
      "SyncEventService - nenhuma integração Google Calendar encontrada para userId",
      userId
    );
    return;
  }
  ...
  const calendar = await buildCalendarClient(tokens, companyId);
