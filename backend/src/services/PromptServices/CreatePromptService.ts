...
const CreatePromptService = async (promptData: PromptData): Promise<Prompt> => {
    ...
    const { name, apiKey, prompt, queueId, maxMessages, provider, model, companyId, toolsEnabled, knowledgeBase } = promptData;
    ...
    try {
        await promptSchema.validate({ name, apiKey, prompt, queueId, maxMessages, provider, model, companyId });
