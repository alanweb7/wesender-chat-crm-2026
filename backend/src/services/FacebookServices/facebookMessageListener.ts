...
export const handleMessage = async (
  ...
  try {
    ...
    if (webhookEvent.message) {
      ...
      console.log(`[FACEBOOK] Mensagem recebida - fromMe: ${fromMe}, bodyMessage: "${bodyMessage}"`);

      // REMOVIDO: Filtro que bloqueava mensagens com \u200e (Facebook Ads usa isso)
      // if (fromMe && /\u200e/.test(bodyMessage)) return;

      if (fromMe) {
        console.log(`[FACEBOOK] Mensagem de echo (sistema) - bodyMessage: "${bodyMessage}"`);
        // Apenas logar, não bloquear
        msgContact = await profilePsid(recipientPsid, token.facebookUserToken);
      } else {
        console.log(`[FACEBOOK] Mensagem de usuário - senderPsid: ${senderPsid}`);
      ...
      // Verificar se mensagem deve processar FlowBuilder menu
      if (!ticket.fromMe && isMenu && !isNaN(message.text)) {
        console.log(`[FACEBOOK] Processando menu - isMenu: ${isMenu}, message.text: "${message.text}"`);
        await ticket.update({
      ...
      console.log(`[FACEBOOK] Verificando condições de integração:
        ...
        - isMenu: ${isMenu}
        - ticket.dataWebhook: ${JSON.stringify(ticket.dataWebhook)}
        - getSession.integrationId: ${getSession.integrationId}
      ...
      if (
        ...
        if (integrations.type === "flowbuilder") {
          ...
          console.log(`[FACEBOOK] Integração não é flowbuilder: ${integrations.type}`);
        }
      } else {
