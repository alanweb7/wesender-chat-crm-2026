...
export const buildNamedKnowledgeBasesSection = async (
  knowledgeBaseIds: number[],
  companyId: number,
  options: ProcessKnowledgeOptions = {}
): Promise<string> => {
  if (!Array.isArray(knowledgeBaseIds) || knowledgeBaseIds.length === 0) return "";

  // import inline para evitar dependência circular
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const KnowledgeBase = require("../../models/KnowledgeBase").default;
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const KnowledgeBaseItemModel = require("../../models/KnowledgeBaseItem").default;

  const bases = await KnowledgeBase.findAll({
    where: { id: knowledgeBaseIds, companyId },
  ...
  for (const base of bases) {
    const items: IKnowledgeBaseItem[] = (base.items || []).map((item: any) => ({
      id: String(item.id),
      type: item.type,
      title: item.title,
    ...
    // Itens de texto não precisam de processamento assíncrono
    const textItems = items.filter(i => i.type === "text");
    const asyncItems = items.filter(i => i.type !== "text");

    const textProcessed: ProcessedKnowledgeBaseItem[] = textItems.map(i => ({
      id: i.id,
      type: "text",
      title: i.title,
    ...
    const asyncProcessed = asyncItems.length
      ? await processKnowledgeBaseItems(asyncItems, { ...options, companyId })
      : [];

    const allProcessed = [...textProcessed, ...asyncProcessed];
    if (!allProcessed.length) continue;

    const blocks = allProcessed.map((item, index) => {
      const header = `  ${index + 1}. [${item.type.toUpperCase()}] ${item.title || item.source || "Sem título"}`;
      return [header, `     ${item.summary}`].join("\n");
    });

    sections.push(`📚 BASE DE CONHECIMENTO "${base.name}":\n${blocks.join("\n\n")}`);
  }
