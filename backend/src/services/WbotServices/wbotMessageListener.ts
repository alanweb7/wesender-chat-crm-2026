...
import { isNil, isArray, isNull } from "lodash";
...
setInterval(() => {
  const cutoff = Date.now() - 60000; // limpa entradas com mais de 60s
...
const flowBuilderMessageGroups = new Map<string, {
  messages: string[];
...
setInterval(() => {
  ...
  flowBuilderMessageGroups.forEach((group, key) => {
  ...
  groupsToRemove.forEach(key => {
    flowBuilderMessageGroups.delete(key);
    console.log(`[FlowBuilder] Limpado grupo antigo: ${key}`);
  });
}, 300000); // 5 minutos

// Função para agrupar mensagens e processar juntas
const groupFlowBuilderMessages = async (
  ticketId: number,
  contactId: number,
  ...
  const groupKey = `${ticketId}_${contactId}`;
  const body = getBodyMessage(msg);

  // Áudio e imagem precisam do dado binário original para a IA processar corretamente:
  // - audioMessage: Whisper transcreve o áudio
  // - imageMessage: Vision API (GPT-4o / Gemini) analisa a imagem visualmente
  // O virtualMsg perderia esses dados binários — bypassar o agrupamento.
  if (msg.message?.audioMessage || msg.message?.imageMessage) {
    await flowbuilderIntegration(msg, wbot, companyId, queueIntegration, ticket, contact, isFirstMsg, isTranfered);
    return;
  }
  
  // Se já existe um grupo, adicionar mensagem
  if (flowBuilderMessageGroups.has(groupKey)) {
    const group = flowBuilderMessageGroups.get(groupKey)!;
    group.messages.push(body);
    
    // Resetar timeout para esperar mais mensagens
    clearTimeout(group.timeout);
    group.timeout = setTimeout(async () => {
      await processGroupedMessages(groupKey, wbot, companyId, queueIntegration, ticket, contact, isFirstMsg, isTranfered);
...
const processGroupedMessages = async (
  ...
  const combinedBody = group.messages.join(' ');
  
  // Criar uma cópia da primeira mensagem para usar como base
  const firstMessage = group.messages[0];
  
  // Criar mensagem virtual combinada
  const virtualMsg = {
    key: {
      id: `grouped_${Date.now()}_${group.ticketId}`,
  ...
  console.log(`[FlowBuilder] Processando ${group.messages.length} mensagens agrupadas para ticket ${group.ticketId}`);
...
import DownloadProfilePic from "../../helpers/DownloadProfilePic";
...
const messageBuffer: Record<number, { 
...
const getBodyButton = (msg: any): string => {
  try {
    ...
    if (msg?.message?.interactiveResponseMessage?.nativeFlowResponseMessage) {
      try {
        ...
        if (paramsJson) {
          ...
          const selectedId = params.id || params.display_text;
          if (selectedId) {
            console.log(`[BOTOES] Botão nativeFlow clicado: ID=${selectedId}`);
    ...
    if (msg?.message?.templateButtonReplyMessage) {
      ...
      const selectedDisplayText = msg.message.templateButtonReplyMessage.selectedDisplayText;
      console.log(`[BOTOES] Botão clicado: ID=${selectedId}, Texto=${selectedDisplayText}`);
    ...
    if (msg?.message?.buttonsResponseMessage) {
      ...
      console.log(`[BOTOES] Botão clicado (buttonsResponseMessage): ID=${selectedButtonId}, Texto=${selectedDisplayText}`);
    ...
    if (!msg.key.fromMe && msg?.message?.interactiveMessage?.nativeFlowMessage?.buttons) {
      const buttons = msg.message.interactiveMessage.nativeFlowMessage.buttons;
      if (buttons && buttons.length > 0) {
        const clickedButton = buttons[0];
        const params = JSON.parse(clickedButton.buttonParamsJson || "{}");
        const displayText = params.display_text;
        console.log(`[BOTOES] Botão clicado (interactiveMessage): Texto=${displayText}`);
        return displayText;
      }
    }

    // Tratamento para viewOnceMessage com botões (iOS)
    if (!msg.key.fromMe && msg?.message?.viewOnceMessage?.message?.interactiveMessage?.nativeFlowMessage?.buttons) {
      const buttons = msg.message.viewOnceMessage.message.interactiveMessage.nativeFlowMessage.buttons;
      if (buttons && buttons.length > 0) {
        const clickedButton = buttons[0];
        const params = JSON.parse(clickedButton.buttonParamsJson || "{}");
        const displayText = params.display_text;
        console.log(`[BOTOES] Botão clicado (viewOnceMessage): Texto=${displayText}`);
        return displayText;
      }
    }
  ...
  try {
    ...
    const types = {
      ...
      interactiveMessage: msg.key.fromMe
        ? (msg.message?.interactiveMessage?.body?.text || getBodyButton(msg))
        : getBodyButton(msg),
...
const getContactMessage = async (msg: any, wbot: Session, senderPn?: string) => {
  ...
  const result = isGroup
    ? {
        id: contactId,  // **CORREÇÃO: Usar contactId (remoteJid do grupo) em vez de senderId**
        name: msg.pushName,  // Temporariamente, será atualizado abaixo
        remoteJidAlt: msg.key.remoteJidAlt,
        addressingMode: msg.key.addressingMode
      }
    : {
  ...
  // **Para grupos, buscar o nome real do grupo**
  if (isGroup && wbot) {
    try {
      const groupMetadata = await wbot.groupMetadata(contactId);
      if (groupMetadata?.subject) {
        result.name = groupMetadata.subject;
        logger.info(`[GRUPO] Nome do grupo obtido: ${groupMetadata.subject}`);
      }
    } catch (err) {
      logger.debug(`[GRUPO] Não foi possível obter metadados do grupo: ${err?.message}`);
    }
  }
...
const verifyContact = async (
  ...
  if (!number && !isGroup) {
    logger.warn("⚠️ Número não extraído para contato não-grupo, tentando fallback", {
      id: msgContact.id,
      remoteJidAlt: msgContact.remoteJidAlt,
      isGroup
    });
    ...
    number = msgContact.remoteJidAlt || msgContact.id?.split('@')[0] || '';
    
    logger.info("🔄 Fallback number:", number);
    
    // **NÃO RETORNAR NULL - CONTINUAR PROCESSAMENTO**
    if (!number) {
      logger.error("❌ Usando ID vazio como último recurso");
      number = msgContact.id || 'unknown';
    }
  }

  const name = msgContact?.name || number;
  logger.info("📝 Nome do contato:", name);
  ...
  if (isGroup) {
    ...
    logger.info("🔗 RemoteJid construído:", remoteJid);
  }

  // ✅ Tentar obter foto com número real (para contatos E grupos)
  if (wbot) {
    try {
      const cdnUrl = await wbot.profilePictureUrl(remoteJid, "image");
      profilePicUrl = await DownloadProfilePic(cdnUrl, companyId, number);
      logger.info(`📸 Profile picture saved for ${isGroup ? 'group' : 'contact'}: ${profilePicUrl}`);
    } catch (err) {
      logger.debug(`Could not get profile picture for ${isGroup ? 'group' : 'contact'}:`, err?.message);
      profilePicUrl = "";
    }
  }
  ...
  logger.info("💾 Salvando contato no banco...");
...
const processQuickMessageCommand = async (
  ...
  try {
    ...
    logger.info(`[processQuickMessageCommand] Detectado comando resp: ${respId}`);
    ...
    if (!quickMessage) {
      logger.warn(`[processQuickMessageCommand] Resposta rápida ID ${respId} não encontrada`);
      return false;
    }

    logger.info(`[processQuickMessageCommand] Resposta rápida encontrada: ${quickMessage.shortcode}`);

    // Buscar o WhatsApp do ticket
    const whatsapp = await ShowWhatsAppService(ticket.whatsappId, ticket.companyId);
    if (!whatsapp) {
      logger.error(`[processQuickMessageCommand] WhatsApp ID ${ticket.whatsappId} não encontrado`);
      return false;
    }
    ...
    if (quickMessage.messageType === "buttons" && quickMessage.buttons?.length) {
      ...
      logger.info(`[processQuickMessageCommand] Botões CTA enviados para "${quickMessage.shortcode}"`);
      return true;
    }
    ...
    if (quickMessage.mediaPath && quickMessage.mediaName) {
      ...
      if (localPath) {
        ...
        logger.info(`[processQuickMessageCommand] Enviando com mídia: ${quickMessage.mediaName}`);
      }
    }

    // Enviar mensagem
    await SendMessage(whatsapp, messageData, false, ticket);
    logger.info(`[processQuickMessageCommand] Resposta rápida "${quickMessage.shortcode}" enviada com sucesso`);

    return true;
  } catch (error) {
    logger.error(`[processQuickMessageCommand] Erro ao processar comando:`, error);
    return false;
  }
};

// Função auxiliar para extrair dados dos botões e carrossel
const getButtonsAndCarouselData = (msg: any): { buttonsData: any, carouselData: any } => {
  try {
    ...
    if (msg?.messageType === "viewOnceMessage" || msg?.message?.viewOnceMessage?.message?.interactiveMessage) {
      ...
      const carouselMsg = msg?.message?.viewOnceMessage?.message?.interactiveMessage?.carouselMessage;
      if (carouselMsg && carouselMsg.cards) {
        carouselData = carouselMsg.cards.map(card => ({
          title: card.header?.title || card.header?.imageMessage?.caption || "",
          description: card.body?.text || "",
          price: card.footer?.text || "",
          image: card.header?.imageMessage?.url || "",
    ...
    if (msg?.messageType === "interactiveMessage" || msg?.message?.interactiveMessage) {
      ...
      const carouselMsg = msg?.message?.interactiveMessage?.carouselMessage;
      if (carouselMsg && carouselMsg.cards) {
        carouselData = carouselMsg.cards.map(card => ({
          title: card.header?.title || card.header?.imageMessage?.caption || "",
          description: card.body?.text || "",
    ...
    return { buttonsData, carouselData };
  } catch (error) {
    logger.error(`Erro ao extrair dados de botões/carrossel: ${error}`);
...
export const verifyMessage = async (
  ...
  // Verificar se é mensagem do usuário (fromMe) e contém comando de resposta rápida
  if (msg.key.fromMe && body) {
    const processed = await processQuickMessageCommand(body, ticket, contact);
    if (processed) {
      // Se processou a resposta rápida, não salva a mensagem original #{ "resp": "X" }
      logger.info(`[verifyMessage] Comando resp processado, mensagem original não será salva`);
      return;
    }
  }
  ...
  // Extrair dados de botões e carrossel
  const { buttonsData, carouselData } = getButtonsAndCarouselData(msg);
  ...
  const newMessage = await CreateMessageService({ messageData, companyId: companyId });
...
const flowbuilderIntegration = async (
  ...
  */
  ...
  // Verificação de horário da conexão antes de disparar qualquer fluxo
  const whatsappHasSchedules = Array.isArray(whatsapp.schedules) && whatsapp.schedules.length > 0;
  if (whatsappHasSchedules && !msg.key.fromMe) {
    const scheduleCheck = await VerifyCurrentSchedule(companyId, 0, whatsapp.id);
    if (!scheduleCheck || scheduleCheck.inActivity === false) {
      // Fora do horário — executar ação configurada e parar
      await ticket.update({ isOutOfHour: true });

      if (whatsapp.outOfHoursFlowId) {
        const outFlow = await FlowBuilderModel.findOne({ where: { id: whatsapp.outOfHoursFlowId } });
        if (outFlow) {
          const mountDataContact = { number: contact.number, name: contact.name, email: contact.email };
          await ActionsWebhookService(
            whatsapp.id, whatsapp.outOfHoursFlowId, ticket.companyId,
            outFlow.flow["nodes"], outFlow.flow["connections"], outFlow.flow["nodes"][0].id,
            null, "", "", null, ticket.id, mountDataContact, msg || null
          );
          return;
        }
      } else if (whatsapp.outOfHoursQueueId) {
        await UpdateTicketService({
        ...
        return;
      } else if (whatsapp.outOfHoursMessage) {
        const body = formatBody(whatsapp.outOfHoursMessage, ticket);
        await wbot.sendMessage(`${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`, { text: body });
        return;
      }
      return;
    }
  ...
  if (ticket.flowWebhook) {
    ...
    // **NOVO: Não executar Flow Builder se ticket já tem fila ou usuário (em atendimento humano)**
    if (ticket.queueId || ticket.userId) {
      console.log(`⚠️ Ticket já tem fila (${ticket.queueId}) ou usuário (${ticket.userId}) - Flow Builder desativado`);
      await ticket.update({ flowWebhook: false });
      return;
    }
    
    // Verificar limite de tentativas do bot (timeSendQueue do whatsapp)
    const whatsapp = await Whatsapp.findByPk(ticket.whatsappId);
    const maxBotAttempts = whatsapp?.timeSendQueue || 0; // timeSendQueue = limite de tentativas
    
    if (maxBotAttempts > 0) {
      const currentAttempts = ticket.botAttempts || 0;
      
      if (currentAttempts >= maxBotAttempts) {
        console.log(`🚫 Limite de tentativas do bot atingido: ${currentAttempts}/${maxBotAttempts}. Encerrando fluxo.`);
        ...
        // Transferir para fila se sendIdQueue estiver configurado
        if (whatsapp?.sendIdQueue) {
          await ticket.update({
            queueId: whatsapp.sendIdQueue,
            status: "pending"
          });
          console.log(`📋 Ticket transferido para fila ${whatsapp.sendIdQueue} por limite de bot`);
        }
      ...
      console.log(`🤖 Tentativa do bot: ${currentAttempts + 1}/${maxBotAttempts}`);
    }
...
export const handleMessageIntegration = async (
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    ...
    if (!isMenu) {
      ...
      await groupFlowBuilderMessages(
        ticket.id,
        contact.id,
        msg,
        wbot,
...
const handleMessage = async (
  ...
  const isGroupInHandle = msg.key.remoteJid?.endsWith("@g.us");
  ...
  const isValidResult = isValidMsg(msg);
  ...
  // De-duplicação: Baileys entrega o mesmo evento 2–4x para botões e mídias.
  // Ignorar se já processamos este msgId nos últimos 60 segundos.
  const msgIdKey = msg.key.id;
  if (msgIdKey && processedMsgIds.has(msgIdKey)) {
    console.log(`[Dedup] Mensagem ${msgIdKey} já processada, ignorando duplicata.`);
    return;
  }
  if (msgIdKey) processedMsgIds.set(msgIdKey, Date.now());
  ...
  try {
    ...
    // Comunidades do WhatsApp têm um formato específico diferente de grupos normais
    // Grupos normais: 120363...@g.us (aceitar)
    // Comunidades: formato específico com @newsletter ou outros identificadores (bloquear se necessário)
    // Por enquanto, aceitar todos os grupos @g.us

    const whatsapp = await ShowWhatsAppService(wbot.id!, companyId);

    console.log("log... 2961");

    // Grupos sempre permitidos - não verificar allowGroup
    ...
    if (isGroup) {
      ...
      console.log("🔵 [GRUPO] Metadados do grupo:", {
        ...
        participantes: grupoMeta.participants?.length
      });
      ...
      console.log("✅ [GRUPO] Contato do grupo criado/atualizado:", {
        id: groupContact.id,
        nome: groupContact.name,
        isGroup: groupContact.isGroup
      });
    }
    ...
    const enableLGPD = settings.enableLGPD;
    ...
    // Conexão com horários próprios configurados — tem prioridade sobre o scheduleType global
    const whatsappHasOwnSchedules = Array.isArray(whatsapp.schedules) && whatsapp.schedules.length > 0;

    if (whatsappHasOwnSchedules) {
      currentSchedule = await VerifyCurrentSchedule(companyId, 0, whatsapp.id);
    } else if (settings.scheduleType === "company") {
    ...
    try {
      if (
        ...
        if (
          ...
          // Ação de fora do horário configurada na conexão
          if (whatsappHasOwnSchedules || settings.scheduleType === "connection") {
            if (whatsapp.outOfHoursFlowId) {
              const outFlow = await FlowBuilderModel.findOne({ where: { id: whatsapp.outOfHoursFlowId } });
              if (outFlow) {
                const nodes = outFlow.flow["nodes"];
                const connections = outFlow.flow["connections"];
                const mountDataContact = { number: contact.number, name: contact.name, email: contact.email };
                await ActionsWebhookService(
                  whatsapp.id, whatsapp.outOfHoursFlowId, ticket.companyId,
                  nodes, connections, outFlow.flow["nodes"][0].id,
                  null, "", "", null, ticket.id, mountDataContact, msg || null
                );
                return;
              }
    ...
    // IA do FlowBuilder: só responde se ticket NÃO tiver fila OU usuário vinculado
    if (ticket.isBot && isOpenai && !isNil(flow) && !ticket.queueId && !ticket.userId) {
      ...
      console.log(`ticket.queueId=${ticket.queueId}, ticket.userId=${ticket.userId}`);
      ...
      if (!messageBuffer[ticket.id]) {
        messageBuffer[ticket.id] = { messages: [] };
      }

      // Adiciona a mensagem completa ao buffer
      messageBuffer[ticket.id].messages.push(msg);
      console.log(`📥 Mensagem adicionada ao buffer (FlowBuilder). Total: ${messageBuffer[ticket.id].messages.length}`);
      ...
      messageBuffer[ticket.id].timeout = setTimeout(async () => {
        try {
          const groupedMessages = messageBuffer[ticket.id].messages;
          delete messageBuffer[ticket.id];

          console.log(`🧠 Processando ${groupedMessages.length} mensagens agrupadas (FlowBuilder)`);

          // Chama a IA com as mensagens agrupadas
          await handleOpenAi(
            openAiSettings,
            groupedMessages, // Envia array de mensagens
            wbot,
            ticket,
            contact,
            mediaSent,
    ...
    if (
      ...
      // Adiciona a mensagem completa ao buffer
      messageBuffer[ticket.id].messages.push(msg);
      console.log(`📥 Mensagem adicionada ao buffer (Conexão). Total: ${messageBuffer[ticket.id].messages.length}`);
      ...
      messageBuffer[ticket.id].timeout = setTimeout(async () => {
        try {
          ...
          console.log(`🧠 Processando ${groupedMessages.length} mensagens agrupadas (Conexão)`);
    ...
    if (
      ...
      (!contact?.acceptAudioMessage ||
        !settings?.acceptAudioMessageContact)
    ) {
      const sentMessage = await wbot.sendMessage(
        `${contact.number}@c.us`,
        {
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
    ...
    messages.forEach(async (message: proto.IWebMessageInfo) => {
      // 🔍 LOG 1: Verificar se mensagem de grupo chegou no listener
            ...
            try {
              ...
              if (isGroupMsg) {
                console.log("🔍 [LOG 3 - REDIS QUEUE] Mensagem de grupo adicionada à fila Redis", {
                  messageId: message.key.id,
                  queueName: `${process.env.DB_NAME}-handleMessage`
                });
              }
  ...
  wbot.ev.on("contacts.update", (contacts: any) => {
    contacts.forEach(async (contact: any) => {
      ...
      if (typeof contact.imgUrl !== "undefined") {
        const cdnUrlContact = contact.imgUrl === ""
          ? ""
          : await wbot!.profilePictureUrl(contact.id!).catch(() => null);
        const newUrl = cdnUrlContact
          ? await DownloadProfilePic(cdnUrlContact, companyId, contact.id.replace(/\D/g, ""))
          : cdnUrlContact;
        ...
        // ✅ Normalizar e validar número (Brasil: 55 + DDD + número, Paraguai: 595 + número)
        const normalizedNumber = normalizePhoneNumber(contactNumber);
        if (!normalizedNumber) {
          logger.error(`❌ Número inválido (não é BR/PY), ignorando: ${contactNumber}`);
          return;
        }
  ...
  wbot.ev.on("groups.update", (groupUpdate: GroupMetadata[]) => {
    ...
    groupUpdate.forEach(async (group: GroupMetadata) => {
      ...
      let profilePicUrl: string = "";
      try {
        const cdnUrlGroup = await wbot.profilePictureUrl(group.id, "image");
        profilePicUrl = await DownloadProfilePic(cdnUrlGroup, companyId, number);
      } catch (e) {
        Sentry.captureException(e);
  ...
  wbot.ev.on("connection.update", (update) => {
    logger.info(`[WHATSAPP MONITOR] Status da conexão atualizado:`, {
    ...
    if (update.connection === "open") {
      logger.info(`[WHATSAPP MONITOR] Conexão aberta com sucesso - WhatsApp ID: ${wbot.id}`);
      WhatsAppMonitor.onConnectionRestored(parseInt(wbot.id as string));
    } else if (update.connection === "close") {
      const reason = update.lastDisconnect?.reason || "Desconhecido";
      logger.error(`[WHATSAPP MONITOR] Conexão fechada - WhatsApp ID: ${wbot.id}, Razão: ${reason}`);
      WhatsAppMonitor.onConnectionLost(parseInt(wbot.id as string));
    }
  });

  // Monitoramento de erros de conexão
  wbot.ev.on("connection.error", (error) => {
    logger.error(`[WHATSAPP MONITOR] Erro de conexão - WhatsApp ID: ${wbot.id}`, error);
    WhatsAppMonitor.onConnectionLost(parseInt(wbot.id as string));
  });
};
