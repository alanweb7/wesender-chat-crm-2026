...
const UpdateTicketService = async ({
  ...
  try {
    ...
    if (whatsappId && whatsappId !== oldWhatsappId) {
      console.log(`🔄 ALTERANDO CONEXÃO: Ticket ${ticket.id} → WhatsApp ${oldWhatsappId} para ${whatsappId}`);
    }

    // **LÓGICA DE VINCULAÇÃO AUTOMÁTICA: Cliente → Usuário Responsável**
    if (isTransfered && queueId && !userId) {
      console.log(`🔄 VERIFICANDO VINCULAÇÃO AUTOMÁTICA: Ticket ${ticket.id} → Fila ${queueId}`);
      
      // Buscar cliente vinculado ao ticket
      ...
      if (client && client.owner) {
        ...
        console.log(`👤 Cliente encontrado: ${client.name} → Responsável: ${clientOwner.name}`);
        
        // Verificar se o usuário responsável está na fila de destino
        const userInTargetQueue = clientOwner.queues?.some((queue: any) => queue.id === queueId);
        
        if (userInTargetQueue) {
          console.log(`✅ VINCULAÇÃO AUTOMÁTICA: Usuário ${clientOwner.name} está na fila ${queueId}`);
          userId = clientOwner.id; // Vincula automaticamente ao usuário responsável
        } else {
          console.log(`❌ VINCULAÇÃO NÃO APLICADA: Usuário ${clientOwner.name} não está na fila ${queueId}`);
        }
      } else {
    ...
    // Só emite delete se não for encerramento de ticket
    // Tickets encerrados devem permanecer visíveis no Kanban até exclusão manual
    const isClosingTicket = ticket.status === "closed" && oldStatus !== "closed";
    
    if (isClosingTicket) {
      console.log(`🔴 KANBAN FIX: Ticket ${ticket.id} sendo ENCERRADO (${oldStatus} -> closed) - NÃO emitindo delete para manter no Kanban`);
    }
