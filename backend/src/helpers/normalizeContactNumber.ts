...
export const normalizePhoneNumber = (value?: string | null): string => {
	...
	const isBrazilNumber = digits.startsWith("55") && digits.length >= 10 && digits.length <= 13; // 10-13 dígitos (com/sem 9)
	...
	const isInternationalNumber = (
		(digits.startsWith("1") && digits.length >= 11 && digits.length <= 11) || // EUA/Canadá
	...
	console.log(`[normalizePhoneNumber] Validando: ${digits} | BR: ${isBrazilNumber} | PY: ${isParaguayNumber} | INT: ${isInternationalNumber} | Len: ${digits.length}`);
	
	if (!isBrazilNumber && !isParaguayNumber && !isInternationalNumber) {
		console.log(`[normalizePhoneNumber] ❌ REJEITADO - Invalid number: ${digits}`);
		return "";
	}

	let country = "Brasil";
	if (isParaguayNumber) country = "Paraguai";
	else if (isInternationalNumber) country = "Internacional";
	
	console.log(`[normalizePhoneNumber] ✅ ACEITO - ${country}: ${digits}`);
	return digits;
};
