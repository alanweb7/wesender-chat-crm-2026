...
import CreateBulkService from "../services/ScheduleServices/CreateBulkService";
...
export const storeBulk = async (req: Request, res: Response): Promise<Response> => {
  ...
  console.log(
    "ScheduleController.storeBulk - criando lembretes em massa",
    "companyId:",
    companyId,
    "sendType:",
    sendType,
  ...
  const { schedules, count } = await CreateBulkService({
    body,
    sendAt,
    companyId,
    userId,
  ...
  // Emitir evento para cada lembrete criado
