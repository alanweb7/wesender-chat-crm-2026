...
const processQuickMessageCommand = async (
  ...
  try {
    ...
    if (quickMessage.mediaPath && quickMessage.mediaName) {
      ...
      if (localPath) {
        // Caminho relativo ao CWD (backend/) — compatível com o padrão do multer/Baileys
        const relativePath = `public/company${ticket.companyId}/quickMessage/${localPath}`;
        messageData.mediaPath = relativePath;
        messageData.mediaName = quickMessage.mediaName;
        console.log(`[processQuickMessageCommand] Enviando com mídia: ${quickMessage.mediaName} → ${relativePath}`);
      }
    }
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (messageType === "buttons" && Array.isArray(buttons) && buttons.length > 0 && ticket.channel === "whatsapp") {
    try {
  ...
  if (quickMessageId && ticket.channel === "whatsapp") {
    try {
