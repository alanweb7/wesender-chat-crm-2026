...
const FlowBuilderMenuModal = ({ open, onSave, onUpdate, data, close }) => {
  ...
  const [menuType, setMenuType] = useState("buttons"); // "buttons" ou "text"
  ...
  useEffect(() => {
    if (open === "edit") {
      ...
      setMenuType(data.data.menuType || "buttons");
      setActiveModal(true);
    } else if (open === "create") {
      setLabels({
        title: "Adicionar menu ao fluxo",
  ...
  const handleSaveContact = async () => {
    if (open === "edit") {
      ...
      onUpdate({
        ...data,
        data: { message: textDig, arrayOption: arrayOption, menuType: menuType }
      });
      return;
    } else if (open === "create") {
      handleClose();
        ...
        <Stack>
          <Stack dividers style={{ gap: "8px", padding: "16px" }}>
            <FormControl fullWidth variant="outlined" style={{ marginBottom: "8px" }}>
              ...
              >
                <MenuItem value="buttons">Botões Interativos</MenuItem>
                <MenuItem value="text">Digitar Opção (1, 2, 3...)</MenuItem>
              </Select>
            </FormControl>
            <TextField
