...
import { MenuItem, FormControl, InputLabel, Select, Typography, Box, Chip, Tooltip, Tabs, Tab, Switch, Paper, Grid, FormControlLabel, Slider } from "@material-ui/core";
...
const openaiModels = [
    { value: "gpt-4o",               label: "GPT-4o — Visão + Áudio",          free: false },
    { value: "gpt-4o-mini",          label: "GPT-4o Mini — Visão (econômico)", free: false },
    { value: "gpt-4o-audio-preview", label: "GPT-4o Audio Preview — Áudio",    free: false },
    { value: "gpt-4-turbo",          label: "GPT-4 Turbo — Visão",             free: false }
];
...
const PromptModal = ({ open, onClose, promptId }) => {
    ...
    useEffect(() => {
        ...
        api.get("/knowledge-bases")
            .then(({ data }) => setAvailableKnowledgeBases(Array.isArray(data) ? data : []))
            .catch(() => {});
    }, [open]);
    ...
    useEffect(() => {
        const fetchPrompt = async () => {
            ...
            try {
                ...
                setTtsModel(data.ttsModel || "tts-1");
                setAudioPercentage(data.audioPercentage || 30);
                setAudioEnabled(data.voice && data.voice !== "texto");
            } catch (err) {
                toastError(err);
            }
        };
    ...
    const getCurrentModels = () => {
        ...
        if (selectedProvider === "grok") return grokModels;
        return openaiModels;
    };
                            ...
                            <Form style={{ width: "100%" }}>
                                ...
                                {activeTab === 2 && (
                                    <>
                                        ...
                                        <Typography className={classes.sectionTitle} style={{ marginTop: 16 }}>
                                            <ThermostatIcon className={classes.sectionIcon} />
                                        ...
                                        <div className={classes.multFieldLine}>
                                            <Field
                                                ...
                                                helperText={touched.temperature && errors.temperature || "0 = mais preciso, 1 = equilibrado, 2 = mais criativo"}
                                                variant="outlined"
                                                margin="dense"
                                                fullWidth
                                                inputProps={{ 
                                ...
                                {activeTab === 4 && (
                                    <>
                                        ...
                                        <Typography variant="body2" color="textSecondary" style={{ marginBottom: 16 }}>
                                            Selecione as bases de conhecimento que este agente poderá acessar. No prompt, instrua a IA quando e como consultar cada base.
                                        </Typography>

                                        {availableKnowledgeBases.length === 0 ? (
                                            <Paper style={{ padding: 32, textAlign: "center", border: "1px dashed #d1d5db" }} elevation={0}>
                                                <LibraryBooksIcon style={{ fontSize: 40, color: "#d1d5db", marginBottom: 8 }} />
                                                <Typography variant="body2" color="textSecondary">
                                                    Nenhuma base de conhecimento cadastrada.
                                                </Typography>
                                                <Typography variant="caption" color="textSecondary">
                                                    Acesse <strong>Automação → Bases de Conhecimento</strong> para criar bases.
                                                </Typography>
                                            </Paper>
                                        ) : (
                                            <Grid container spacing={2}>
                                                {availableKnowledgeBases.map(base => {
                                                    const ids = values.knowledgeBaseIds || [];
                                                    const isSelected = ids.includes(base.id);
                                                    return (
                                                            ...
                                                            <Paper
                                                                ...
                                                                onClick={() => {
                                                                    const next = isSelected
                                                                        ...
                                                                        : [...ids, base.id];
                                                                    setFieldValue("knowledgeBaseIds", next);
                                                                }}
                                                            >
                                                                <Box display="flex" alignItems="flex-start" style={{ gap: 12 }}>
                                                                    <LibraryBooksIcon style={{ color: isSelected ? "#6366f1" : "#9ca3af", marginTop: 2 }} />
                                                                    <Box flex={1}>
                                                                        <Typography variant="subtitle2" style={{ fontWeight: 600, color: isSelected ? "#4f46e5" : "#111827" }}>
                                                                            {base.name}
                                                                        </Typography>
                                                                        {base.description && (
                                                                        ...
                                                                        <Chip
                                                                            ...
                                                                            size="small"
                                                                            style={{ marginTop: 6, height: 20, fontSize: 11 }}
                                                                        />
                                                                    </Box>
                                                                    {isSelected && (
                                                                        <Chip label="Vinculada" size="small" style={{ backgroundColor: "#6366f1", color: "#fff", height: 20, fontSize: 11 }} />
                                                                    )}
                                                                </Box>
                                                            </Paper>
                                                        </Grid>
                                ...
                                {activeTab === 5 && (
                                    <>
                                        ...
                                        {(selectedProvider === "gemini" || selectedProvider === "grok") && (
                                            <Paper style={{ padding: 16, marginBottom: 16, backgroundColor: "#fff8e1", border: "1px solid #ffe082" }}>
                                                <Typography variant="subtitle2" style={{ marginBottom: 8, fontWeight: 600, color: "#f57f17" }}>
                                                    🎤 OpenAI API Key — Transcrição de Áudio (Whisper)
                                                </Typography>
                                                <Typography variant="body2" color="textSecondary" style={{ marginBottom: 12 }}>
                                                    {selectedProvider === "gemini" ? "Gemini" : "Grok"} não possui transcrição de áudio. Informe uma chave OpenAI para usar o Whisper ao receber mensagens de voz dos clientes.
                                                </Typography>
                                                <Field
                                                    as={TextField}
                                                    ...
                                                    type="password"
                                                    placeholder="sk-..."
                                                    helperText="Necessária para transcrever áudios recebidos via WhatsApp"
                                                />
                                            </Paper>
                                        )}
