...
const MessageInput = ({ ticketId, ticketStatus, droppedFiles, contactId, ticketChannel, notificameHub }) => {
  ...
  const handleUploadMedia = async (mediasUpload) => {
    ...
    try {
      ...
      console.error("[MessageInput] Erro no upload de mídia:", err);
      // Ignora erros 403 de avatar (pps.whatsapp.net) para não quebrar o fluxo
      if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
        console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
      } else {
        toastError(err);
      }
    } finally {
  ...
  const handleSendContatcMessage = async (vcard) => {
    ...
    try {
      ...
      console.error("[MessageInput] Erro no envio de contato:", err);
      if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
        console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
      } else {
        toastError(err);
  ...
  const handleSendMessage = async () => {
   ...
   try {
      ...
      console.error("[MessageInput] Erro no envio de mensagem:", err);
      if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
        console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
      } else {
        toastError(err);
...
const handleUploadCamera = async (blob) => {
  ...
  try {
    ...
    console.error("[MessageInput] Erro no upload da câmera:", err);
    if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
      console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
    } else {
      toastError(err);
  ...
  const handleUploadAudio = async () => {
    ...
    try {
      ...
      console.error("[MessageInput] Erro no upload de áudio:", err);
      if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
        console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
      } else {
        toastError(err);
  ...
  const handleUploadQuickMessageMedia = async (blob, caption) => {
    setLoading(true);
    try {
      const formData = new FormData();
      formData.append("medias", blob);
      ...
      console.error("[MessageInput] Erro no upload de mídia rápida:", err);
      if (err?.response?.status === 403 && err?.response?.data?.error?.includes?.("pps.whatsapp.net")) {
        console.warn("[MessageInput] Ignorando erro 403 de avatar (pps.whatsapp.net)");
      } else {
        toastError(err);
