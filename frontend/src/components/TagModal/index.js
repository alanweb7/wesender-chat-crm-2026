...
const TagModal = ({ open, onClose, tagId, kanban }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tagsRes, queuesRes, usersRes, flowsRes] = await Promise.all([
          api.get("/tags"),
          api.get("/queue"),
          api.get("/users"),
          api.get("/flowbuilder")
        ...
        setAllQueues(Array.isArray(queuesRes.data) ? queuesRes.data : []);
        setAllUsers(usersRes.data?.users || []);
        setAllFlows(flowsRes.data?.flows || []);
      } catch (err) {
  ...
  const handleRemoveAutoAction = (actionId) => {
    setAutoActions(autoActions.filter(action => action.id !== actionId));
  };

  const handleUpdateAutoAction = (actionId, field, value) => {
    setAutoActions(autoActions.map(action => 
                    ...
                    <Grid container spacing={2}>
                      ...
                      {kanban === 1 && (
                        <>
                          <Grid item xs={12}>
                            <Typography variant="h6" className={classes.sectionTitle}>
                              <Gradient /> Ações Automáticas
                            </Typography>
                          </Grid>
                          
                          <Grid item xs={12}>
                            <Typography variant="body2" color="textSecondary" style={{ marginBottom: 12 }}>
                              Configure ações que serão executadas automaticamente quando um ticket for vinculado a esta tag Kanban.
                            </Typography>
                          </Grid>
                          ...
                          {autoActions.map((action, index) => (
                            <Grid item xs={12} key={action.id} style={{ 
                              ...
                              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                                ...
                                <IconButton
                                  size="small"
                                  onClick={() => handleRemoveAutoAction(action.id)}
                                  color="error"
                                >
                              ...
                              <Grid container spacing={2}>
                                <Grid item xs={12} md={6}>
                                  <FormControl variant="outlined" fullWidth size="small">
                                    ...
                                    <Select
                                      value={action.type}
                                      onChange={(e) => handleUpdateAutoAction(action.id, 'type', e.target.value)}
                                      label="Tipo de Ação"
                                    >
                                      <MenuItem value="">Selecione...</MenuItem>
                                      <MenuItem value="transferQueue">Transferir para Fila</MenuItem>
                                      <MenuItem value="transferUser">Transferir para Atendente</MenuItem>
                                      <MenuItem value="transferQueueUser">Transferir para Fila + Atendente</MenuItem>
                                      <MenuItem value="startFlow">Iniciar Fluxo</MenuItem>
                                      <MenuItem value="scheduleMessage">Follow Up</MenuItem>
                                    </Select>
                                  </FormControl>
                                </Grid>

                                {(action.type === 'transferQueue' || action.type === 'transferQueueUser') && (
                                  <Grid item xs={12} md={6}>
                                    <FormControl variant="outlined" fullWidth size="small">
                                      <InputLabel>Fila</InputLabel>
                                      <Select
                                        value={action.queueId}
                                        onChange={(e) => handleUpdateAutoAction(action.id, 'queueId', e.target.value)}
                                        label="Fila"
                                      >
                                        <MenuItem value="">Selecione...</MenuItem>
                                        {allQueues.map(queue => (
                                ...
                                {(action.type === 'transferUser' || action.type === 'transferQueueUser') && (
                                  <Grid item xs={12} md={6}>
                                    <FormControl variant="outlined" fullWidth size="small">
                                      <InputLabel>Atendente</InputLabel>
                                      <Select
                                        value={action.userId}
                                        onChange={(e) => handleUpdateAutoAction(action.id, 'userId', e.target.value)}
                                        label="Atendente"
                                      >
                                        <MenuItem value="">Selecione...</MenuItem>
                                        {allUsers.map(user => (
                                ...
                                {action.type === 'startFlow' && (
                                  <Grid item xs={12} md={6}>
                                    <FormControl variant="outlined" fullWidth size="small">
                                      ...
                                      <Select
                                        value={action.flowId}
                                        onChange={(e) => handleUpdateAutoAction(action.id, 'flowId', e.target.value)}
                                        label="Fluxo"
                                      >
                                        <MenuItem value="">Selecione...</MenuItem>
                                        {allFlows.map(flow => (
                                  ...
                                  <>
                                    <Grid item xs={12} md={6}>
                                      <TextField
                                        ...
                                        fullWidth
                                        size="small"
                                        value={action.scheduleDelay}
                                        onChange={(e) => handleUpdateAutoAction(action.id, 'scheduleDelay', e.target.value)}
                                      />
                                    </Grid>
                                    <Grid item xs={12} md={6}>
                                      <FormControl variant="outlined" fullWidth size="small">
                                        ...
                                        <Select
                                          value={action.scheduleDelayUnit}
                                          onChange={(e) => handleUpdateAutoAction(action.id, 'scheduleDelayUnit', e.target.value)}
                                          label="Unidade"
                                        >
                                    ...
                                    <Grid item xs={12}>
                                      <TextField
                                        ...
                                        fullWidth
                                        size="small"
                                        value={action.scheduleMessageText}
                                        onChange={(e) => handleUpdateAutoAction(action.id, 'scheduleMessageText', e.target.value)}
                                    ...
                                    <Grid item xs={12}>
                                      <Box display="flex" alignItems="center">
                                        <Checkbox
                                          checked={action.scheduleStartFlow}
                                          onChange={(e) => handleUpdateAutoAction(action.id, 'scheduleStartFlow', e.target.checked)}
                                          color="primary"
                                        />
                                        <Typography variant="body2">
                                          Iniciar fluxo após enviar mensagem
                                        </Typography>
                                      </Box>
                                    </Grid>
                                    {action.scheduleStartFlow && (
                                      <Grid item xs={12} md={6}>
                                        <FormControl variant="outlined" fullWidth size="small">
                                          <InputLabel>Fluxo</InputLabel>
                                          <Select
                                            value={action.scheduleFlowId}
                                            onChange={(e) => handleUpdateAutoAction(action.id, 'scheduleFlowId', e.target.value)}
                                            label="Fluxo"
                                          >
                                            <MenuItem value="">Selecione...</MenuItem>
                                            {allFlows.map(flow => (
