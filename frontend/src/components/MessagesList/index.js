...
import InteractiveButtonsPreview from "../InteractiveButtonsPreview";
...
const COLLAPSE_THRESHOLD = 600; // caracteres

const MessageBody = ({ text, deleted, lgpd }) => {
  const [expanded, setExpanded] = useState(false);
  const body = (lgpd && deleted) ? "🚫 _Mensagem apagada_ " : text;
  const isLong = body && body.length > COLLAPSE_THRESHOLD;
  const displayed = isLong && !expanded ? body.slice(0, COLLAPSE_THRESHOLD) : body;

  return (
    <>
      <MarkdownWrapper>{displayed}</MarkdownWrapper>
      {isLong && (
        <span
          onClick={() => setExpanded(e => !e)}
          style={{ display: "block", color: "#2563eb", fontSize: "0.82rem", fontWeight: 600, cursor: "pointer", marginTop: 2, userSelect: "none" }}
        >
...
const renderQuotedMessage = (message) => {
  ...
  const renderQuotedContent = () => {
    ...
    if (quoted.mediaType === "video" && quoted.mediaUrl) {
      return (
        <video className={classes.messageMedia} src={getMediaUrl(quoted.mediaUrl)} controls />
      );
    }
    ...
    if (quoted.mediaType === "sticker" && quoted.mediaUrl) {
      return <img src={getMediaUrl(quoted.mediaUrl)} alt="Sticker" className={classes.messageMedia} loading="lazy" />;
    }
      ...
      return (
        <div className={classes.downloadMedia}>
          <Button
            ...
            variant="outlined"
            target="_blank"
            href={getMediaUrl(quoted.mediaUrl)}
          >
            Download
          </Button>
        </div>
