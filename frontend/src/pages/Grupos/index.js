            ...
            <IconButton
              ...
              onClick={async () => {
                try {
                  ...
                  history.push(`/grupos/${selectedTicket.id}`);
...
useEffect(() => {
  ...
  const shouldDisplayTicketRef = useRef((ticket) => {
    ...
    // **REGRAS ESPECIAIS PARA GRUPOS**: Todos com allowGroup podem ver/responder grupos (com ou sem usuário)
    if (ticket.isGroup === true && user?.allowGroup === true) {
      canSeeTicket = true; // Qualquer usuário com allowGroup pode ver QUALQUER grupo
  ...
  // **DESLIGADO: Sockets causando loop infinito de notificações**
  // useEffect(() => {
  ...
  //       // **CORREÇÃO: Verificar se é grupo (única aba existente)**
  //       const belongsToCurrentTab = data.ticket?.isGroup === true;
  ...
  //       // **CORREÇÃO: Sempre verificar se é grupo (única aba existente)**
  //       const belongsToCurrentTab = data.ticket?.isGroup === true;
  ...
  // **REATIVADO: Sockets para atualização em tempo real de mensagens**
  useEffect(() => {
    const socket = socketConnection({ companyId: user.companyId });
    ...
    socket.on(`company-${user.companyId}-appMessage`, (data) => {
      if (data.action === "create") {
        // Verificar se é mensagem de um grupo
        const isGroupMessage = data.ticket?.isGroup === true;
        
        // Se não for grupo ou usuário não tem permissão, ignorar
        if (!isGroupMessage || !user?.allowGroup) {
          return;
        }
        
        // Atualiza a lista de tickets (grupos)
        setTickets((prevTickets) => {
          const ticketIndex = prevTickets.findIndex(t => t.id === data.message.ticketId);
          
          if (ticketIndex !== -1) {
            // Grupo já existe na lista - atualiza
            const updatedTickets = [...prevTickets];
            const ticket = { ...updatedTickets[ticketIndex] };
            ...
            // Atualiza contador de não lidas se não for mensagem enviada por mim
            if (!data.message.fromMe) {
              ticket.unreadMessages = (ticket.unreadMessages || 0) + 1;
              
              // Notificação somente se for mensagem de outro
              const ticketChannel = ticket.channel;
              if (!isAdAutomaticMessage(data.message, ticketChannel)) {
                playNotificationSound();
                showDesktopNotification(
        ...
        // Se o grupo estiver selecionado, atualiza as mensagens em tempo real
        const currentTicket = selectedTicketRef.current;
        if (currentTicket && data.message.ticketId === currentTicket.id) {
          // Verifica se o usuário pode ver este ticket
          if (!ticketMatchesCurrentFilters(data.ticket || currentTicket)) {
            return;
          }
          ...
          // Adiciona mensagem à lista em tempo real
          setMessages(prevMessages => {
            // Evita duplicação verificando se a mensagem já existe
            const messageExists = prevMessages.some(msg => msg.id === data.message.id);
            if (!messageExists) {
          ...
          // Scroll para o final se a mensagem for de outra pessoa
          if (!data.message.fromMe) {
            setTimeout(() => {
              if (messagesEndRef.current) {
                messagesEndRef.current.scrollIntoView({ behavior: "smooth" });
    ...
    // Listener para atualização de tickets (status, etc)
    socket.on(`company-${user.companyId}-ticket`, (data) => {
      if (data.action === "update" || data.action === "create") {
        // Verificar se é grupo
        if (data.ticket?.isGroup !== true) return;
        
        // Verificar se pode ver este ticket
        if (!ticketMatchesCurrentFilters(data.ticket)) {
          return;
        }
        ...
        setTickets((prevTickets) => {
          ...
          if (ticketIndex !== -1) {
            // Ticket já existe - atualiza
            const updatedTickets = [...prevTickets];
            updatedTickets[ticketIndex] = data.ticket;
            
            // Reordena se updatedAt mudou
            const result = updatedTickets.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
            return [...result];
          }
    ...
    // Listener para atualização de contato
    socket.on(`company-${user.companyId}-contact`, (data) => {
      if (data.action === "update") {
        const currentTicket = selectedTicketRef.current;
  ...
  }, [user.companyId, user?.allowGroup]);
  ...
  // TAB_CONFIG simplificado para Grupos - apenas uma aba
  ...
  const ticketMatchesCurrentFilters = useCallback((ticket) => {
    ...
    // **REGRAS ESPECIAIS PARA GRUPOS**: Se for grupo e usuário tem allowGroup, permite ver SEM FILTROS
    if (ticket.isGroup === true && user?.allowGroup === true) {
      return true; // Permite ver QUALQUER grupo, ignorando TODOS os filtros
    }

    // **GRUPOS: Se for grupo, não pode ter fila nem usuário vinculado**
    if (ticket.isGroup === true) {
      // Grupos não devem ter fila nem usuário, mas se tiver, ignora para compatibilidade
      return true;
    }
    ...
    if (!showAllTickets) {
      ...
      if (messageDirectionFilter !== "all") {
        if (messageDirectionFilter === "waiting_customer" && ticket.lastMessage && ticket.lastMessage !== "") {
          return false;
        }
        if (messageDirectionFilter === "waiting_agent" && (!ticket.lastMessage || ticket.lastMessage === "")) {
          return false;
        }
      }
    } else {
      // Quando "ver todos" está ativo, mostrar todos os tickets exceto os de outros usuários (a menos que tenha permissão)
      if (ticket.userId && ticket.userId !== user?.id && !canSeeOtherUsersTickets) {
        return false;
      }
  ...
  const loadTickets = useCallback(async () => {
    console.log("[GRUPOS] loadTickets chamado - user.allowGroup:", user?.allowGroup);
    
    try {
      setLoading(true);
      
      // Verificar se usuário tem permissão para acessar grupos
      if (!user?.allowGroup) {
        console.log("[GRUPOS] Usuário não tem permissão allowGroup");
        setTickets([]);
        setLoading(false);
        return;
      }
      
      // **GRUPOS SEM FILTROS**: Para grupos, não aplicar filtros de fila/usuário/tags
      const params = {
        status: "group",  // Backend usa status="group" para buscar grupos
        isGroup: true     // Mantém isGroup como fallback
      };

      const { data } = await api.get("/tickets", { params });
      
      console.log("[GRUPOS DEBUG] Total tickets da API:", data.tickets?.length || 0);
      console.log("[GRUPOS DEBUG] Tickets brutos:", data.tickets);
      
      // Verificar se tickets são grupos por diferentes propriedades
      console.log("[GRUPOS DEBUG] Verificando propriedades dos tickets:");
      
      // Mostrar alguns exemplos de tickets para ver suas propriedades
      const sampleTickets = (data.tickets || []).slice(0, 5);
      sampleTickets.forEach((ticket, index) => {
        console.log(`[GRUPOS DEBUG] Ticket ${index}:`, {
          id: ticket.id,
      ...
      // Verificar se usuário tem permissão para ver todas as conexões
      // ADMINISTRADOR e GERENTE veem todos os grupos
      // ATENDENTE e PROFISSIONAL veem apenas grupos da sua conexão
      const userType = (user?.userType || "").toLowerCase();
      const isAdminOrManager = userType === "administrador" || userType === "gerente";
      const canViewAllConnections = isAdminOrManager || user?.allowConnections === "enabled";
      const userWhatsappId = user?.whatsappId;
      
      console.log("[GRUPOS DEBUG] Filtro de conexão:", {
        canViewAllConnections,
        userWhatsappId,
      ...
      // Tentar diferentes filtros para encontrar grupos
      const groupTickets = (data.tickets || []).filter(ticket => {
        // Verificar múltiplas propriedades que podem indicar grupo
        const isGroupTicket = ticket.isGroup === true || 
               ticket.contact?.name?.includes("Grupo") ||
               ticket.channel === "whatsapp" && ticket.contact?.name?.includes("Grupo");
        
        // Se não for grupo, ignorar
        if (!isGroupTicket) {
          console.log(`[GRUPOS DEBUG] Ticket ${ticket.id} não é grupo - ignorado`);
          return false;
        }
        
        // Obter whatsappId do ticket (pode estar em whatsappId ou whatsapp.id)
        const ticketWhatsappId = ticket.whatsappId || ticket.whatsapp?.id;
        
        // Log detalhado de cada grupo
        console.log(`[GRUPOS DEBUG] Analisando grupo ${ticket.id}:`, {
          name: ticket.contact?.name,
          ticketWhatsappId: ticketWhatsappId,
          whatsappName: ticket.whatsapp?.name,
          canViewAll: canViewAllConnections,
          userWhatsappId: userWhatsappId
        });
        
        // Se pode ver todas as conexões (ADMINISTRADOR ou GERENTE), mostrar todos os grupos
        if (canViewAllConnections) {
          console.log(`[GRUPOS DEBUG] Ticket ${ticket.id} - PERMITIDO (${userType} ou allowConnections)`);
          return true;
        }
        
        // ATENDENTE ou PROFISSIONAL: filtrar apenas grupos da conexão vinculada
        if (userWhatsappId) {
          const matches = ticketWhatsappId === userWhatsappId;
          console.log(`[GRUPOS DEBUG] Ticket ${ticket.id} - ${matches ? 'PERMITIDO' : 'BLOQUEADO'} (ticketWhatsappId: ${ticketWhatsappId} vs user: ${userWhatsappId})`);
          return matches;
        }
        
        // Se não tem whatsappId definido, mostrar todos (fallback)
        console.log(`[GRUPOS DEBUG] Ticket ${ticket.id} - PERMITIDO (sem whatsappId definido - fallback)`);
        return true;
      });
      
      console.log("[GRUPOS DEBUG] Total grupos após filtro:", groupTickets.length);
      console.log("[GRUPOS DEBUG] IDs dos grupos filtrados:", groupTickets.map(t => ({ id: t.id, name: t.contact?.name, whatsapp: t.whatsapp?.name })));
  ...
  }, [user?.allowGroup, user?.profile, user?.allowConnections, user?.whatsappId]);
  ...
  const loadTicket = async (id) => {
    try {
      ...
      // **REGRAS ESPECIAIS PARA GRUPOS**: Permitir acesso se for grupo e usuário tem allowGroup
      if (data.isGroup === true && user?.allowGroup === true) {
        // Permite acesso a grupos mesmo que seja de outro usuário
        setSelectedTicket(data);
        loadMessages(id);
        return;
      }
      
      // **CRÍTICO: Para outros tickets, verificar se usuário pode ver**
      ...
      if (err?.response?.status === 403) {
        // **REGRAS ESPECIAIS PARA GRUPOS**: Se for 403 mas é grupo, tenta acessar assim mesmo
        // Isso pode acontecer quando o backend bloqueia acesso de outros usuários
        // Mas para grupos queremos permitir acesso compartilhado
        console.log("[GRUPOS] Erro 403, tentando acesso compartilhado...");
        
        // Tenta carregar sem verificação de permissão (apenas para grupos)
        try {
          const { data: ticketData } = await api.get(`/tickets/${id}`);
          if (ticketData.isGroup === true && user?.allowGroup === true) {
            setSelectedTicket(ticketData);
            loadMessages(id);
            return;
          }
        } catch (retryErr) {
          console.log("[GRUPOS] Falha no acesso compartilhado:", retryErr);
        }
  ...
  const handleReturnTicket = async () => {
    ...
    try {
      ...
      setSelectedTicket(prev => prev ? { ...prev, status: "open", userId: null } : prev);
      ...
      {shouldShowList && (
        <div className={`${classes.sidebar} ${isMobile ? classes.sidebarMobile : ""}`}>
          ...
          <div style={{ 
            ...
            <Typography style={{ fontSize: 18, fontWeight: 500, color: "#111b21" }}>
              Grupos ({tickets.length})
            </Typography>
            <Badge 
          ...
          <div className={classes.ticketsList}>
            {loading ? (
              ...
              tickets.map((ticket) => (
                ...
                >
                  <Avatar
                    src={ticket.isGroup ? ticket.contact?.profilePicUrl : ticket.contact?.profilePicUrl}
                    className={classes.ticketAvatar}
                  >
                    {ticket.isGroup ? (ticket.contact?.name || "Grupo").charAt(0) : (ticket.contact?.name || "Sem nome").charAt(0)}
                  </Avatar>
                  <div className={classes.ticketInfo}>
                    <Typography className={classes.ticketName}>
                      {ticket.isGroup ? (ticket.contact?.name || "Grupo") : (ticket.contact?.name || "Sem nome")}
                    </Typography>
