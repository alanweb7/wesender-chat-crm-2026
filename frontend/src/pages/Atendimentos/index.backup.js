...
const Atendimentos = () => {
	...
	const handleConvertToClient = async (emailFromModal = '') => {
		...
		try {
			...
			if (existingClients && existingClients.length > 0) {
				...
				console.log('🔄 Ticket atualizado com cliente:', updatedTicket.crmClient);
			...
			setTickets(prevTickets => {
				...
				console.log('🔄 Lista de tickets atualizada:', newTickets.length);
				return newTickets;
			});
	...
	useEffect(() => {
		if (selectedTicket) {
			const client = selectedTicket?.crmClient || selectedTicket?.contact?.crmClients?.[0];
			console.log('Ticket atualizado - Cliente:', client);
		}
	}, [selectedTicket]);
...
useEffect(() => {
	...
	useEffect(() => {
		const loadQuickMessages = async () => {
			try {
				...
				console.log("🔍 Respostas rápidas carregadas do backend:", messages);
				console.log("🔍 Total de mensagens:", messages.length);
				if (messages.length > 0) {
					console.log("🔍 Primeira mensagem (exemplo):", messages[0]);
					console.log("🔍 Campos da primeira mensagem:", Object.keys(messages[0]));
					console.log("🔍 mediaPath:", messages[0].mediaPath);
					console.log("🔍 mediaName:", messages[0].mediaName);
					console.log("🔍 mediaType:", messages[0].mediaType);
				}
	...
	const handleSelectQuickMessage = async (quickMessage) => {
		console.log("🔔 handleSelectQuickMessage CHAMADO!", new Date().toISOString());
		setQuickMessagesOpen(false);
		
		if (!selectedTicket) {
			console.log("⚠️ Sem ticket selecionado, inserindo no input");
			setInputMessage(quickMessage.message || "");
			return;
		}
		
		console.log("📤 Enviando resposta rápida:", quickMessage);
		console.log("📎 mediaPath:", quickMessage.mediaPath);
		console.log("📝 mediaName:", quickMessage.mediaName);
		console.log("🎯 mediaType:", quickMessage.mediaType);
		console.log("💬 message:", quickMessage.message);
		
		try {
			// Se a resposta rápida tem arquivo, envia arquivo COM texto como legenda
			if (quickMessage.mediaPath) {
				console.log("🔄 Baixando arquivo de:", quickMessage.mediaPath);
				...
				console.log("✅ Arquivo baixado - Tamanho:", blob.size, "Tipo:", blob.type);
				
				// Criar um File a partir do blob
				const fileName = quickMessage.mediaName || "arquivo";
				const file = new File([blob], fileName, { type: blob.type });
				console.log("📦 File criado:", file.name, file.size, file.type);
				
				// Enviar mídia COM texto como legenda (tudo em uma requisição)
				const formData = new FormData();
				formData.append("medias", file);
				formData.append("body", quickMessage.message || "");
				
				console.log("🚀 Enviando arquivo + texto via FormData...");
				const mediaResponse = await api.post(`/messages/${selectedTicket.id}`, formData, {
					headers: {
						"Content-Type": "multipart/form-data",
					},
				});
				console.log("✅ Arquivo + texto enviados com sucesso:", mediaResponse.data);
			} else if (quickMessage.message && quickMessage.message.trim()) {
				// Se não tem arquivo, envia só o texto
				console.log("📝 Enviando apenas texto:", quickMessage.message);
				await api.post(`/messages/${selectedTicket.id}`, {
